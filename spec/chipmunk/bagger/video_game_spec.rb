...
RSpec.describe Chipmunk::Bagger::VideoGame do
  let(:external_id) { "12345" }
  let(:fakeuuid) { "fakeuuid" }
  let(:fixture_data) { fixture("video_game", "pre-chipmunk") }
    ...
    context "with stubbed Chipmunk::Bag" do
      ...
      it "adds the expected metadata tags" do
        expect(bag).to receive(:write_chipmunk_info).with(
          ...
          "Chipmunk-Content-Type" => "video_game",
          "Bag-ID" => fakeuuid
        )
      ...
      it "raises an exception if the bag is not valid" do
        ...
        expect { make_bag("video_game") }.to raise_exception(RuntimeError)
      end
    end
